<template>
    <form>
        <div class="weights weights-add md-layout">
...
export default {
    ...
    methods: {
        saveWeight () {
            Vue.axios.post('weights/add', this.weight)
                .then((response) => {
                    this.setSnackbarText('Weight added successfully')
                    this.setSnackbarShow(true)
                    this.$router.push('/weights/' + response.data.hashid)
                })
                .catch((response) => {
