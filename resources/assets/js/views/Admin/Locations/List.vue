<template>
    <div>
        <md-table v-model="locations" md-sort="created_at" md-sort-order="desc" md-card md-fixed-header>
        <md-table-toolbar>
            <div class="md-toolbar-section-start">
                <h1 class="md-title">{{ Translate('locations') }}</h1>
            </div>
        ...
        <md-table-empty-state>
            <md-button class="md-primary md-raised" to="/locations/add">{{ Translate('add.new') }}</md-button>
        </md-table-empty-state>
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
            ...
            <md-table-cell md-label="">
                <md-button class="md-icon-button md-primary" :to="'/locations/' + item.hashid">
...
import { mapMutations } from 'vuex'
  ...
  export default {
    ...
    created() {
        this.setPageTitle(this.Translate('locations'))
    ...
    methods: {
        searchOnTable () {
            this.locations = this.search(this.locations, this.search)
        },
