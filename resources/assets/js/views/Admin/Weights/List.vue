<template>
    <div>
        <md-table v-model="weights" md-sort="created_at" md-sort-order="desc" md-card md-fixed-header>
        <md-table-toolbar>
            <div class="md-toolbar-section-start">
                <h1 class="md-title">{{ Translate('weights') }}</h1>
            </div>
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
            ...
            <md-table-cell md-label="">
                <md-button class="md-icon-button md-primary" :to="'/weights/' + item.hashid">
...
import { mapMutations } from 'vuex'
  ...
  export default {
    ...
    created() {
        this.setPageTitle(this.Translate('weights'))
    ...
    methods: {
        searchOnTable () {
            this.weights = this.search(this.weights, this.search)
        },
