<template>
    <div>
        <md-table v-model="cities" md-sort="created_at" md-sort-order="desc" md-card md-fixed-header>
        <md-table-toolbar>
            <div class="md-toolbar-section-start">
                <h1 class="md-title">{{ Translate('all.cities') }}</h1>
            </div>
        ...
        <md-table-empty-state>
            <md-button class="md-primary md-raised" to="/cities/add">{{ Translate('add.new.city') }}</md-button>
        </md-table-empty-state>
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
            ...
            <md-table-cell md-label="">
                <md-button class="md-icon-button md-primary" :to="'/cities/' + item.hashid">
...
import { mapMutations } from 'vuex'
  ...
  export default {
    ...
    created() {
        this.setPageTitle(this.Translate('cities'))
    ...
    methods: {
        searchOnTable () {
            this.cities = this.search(this.cities, this.search)
        },
