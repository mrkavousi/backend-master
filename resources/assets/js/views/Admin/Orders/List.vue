<template>
    <div>
        <md-table v-model="orders" md-sort="created_at" md-sort-order="desc" md-card md-fixed-header>
        <md-table-toolbar>
            <div class="md-toolbar-section-start">
                <h1 class="md-title">{{ Translate('all.orders') }}</h1>
            </div>
        ...
        <md-table-empty-state>
            <md-button class="md-primary md-raised" to="/orders/add">{{ Translate('add.new.order') }}</md-button>
        </md-table-empty-state>
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
            ...
            <md-table-cell md-label="">
                <md-button class="md-icon-button md-primary" :to="'/orders/' + item.hashid">
...
import { mapMutations } from 'vuex'
  ...
  export default {
    ...
    created() {
        this.setPageTitle(this.Translate('orders'))
    ...
    methods: {
        searchOnTable () {
            this.orders = this.search(this.orders, this.search)
        },
