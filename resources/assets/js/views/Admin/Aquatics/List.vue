<template>
    <div>
        <md-table v-model="aquatics" md-sort="created_at" md-sort-order="desc" md-card md-fixed-header>
        <md-table-toolbar>
            <div class="md-toolbar-section-start">
                <h1 class="md-title">{{ Translate('all.aquatics') }}</h1>
            </div>
        ...
        <md-table-empty-state>
            <md-button class="md-primary md-raised" to="/aquatics/add">{{ Translate('add.new.aquatic') }}</md-button>
        </md-table-empty-state>
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
            ...
            <md-table-cell md-label="">
                <md-button class="md-icon-button md-primary" :to="'/aquatics/' + item.hashid">
...
import { mapMutations } from 'vuex'
  ...
  export default {
    ...
    created() {
        this.setPageTitle(this.Translate('aquatics'))
    ...
    methods: {
        searchOnTable () {
            this.aquatics = this.search(this.aquatics, this.search)
        },

        search (aquatics, search) {
            if (search) {
                return aquatics
            }
