<template>
    <form>
        <div class="shipments shipments-add order-shipment-add md-layout">
                    ...
                    <md-card-content>
                        ...
                        <div v-if="shipment.packages.length" class="md-layout mb-4">
                            ...
                            <md-card class="md-dark md-layout-item md-size-100 mb-2 card-selected-package" md-with-hover v-for="(pack, index) in shipment.packages" :key="index">
                                ...
                                <md-card-content>
                                    {{ Translate('from') }} {{ Translate('unload') }} 
                                    <b><router-link :to="'/projects/' + pack.unload_process.project_hashid + '/processes/' + pack.unload_process.hashid">{{pack.unload_process.metadatas.name || 'Untitled'}} ({{pack.unload_process.hashid}})</router-link></b><br>
                                    {{ Translate('storaged.in') }}  <b>{{pack.inventory.storage.name}}</b> | {{ Translate('code') }}: {{pack.hashid}}
                                </md-card-content>
...
export default {
    ...
    created() {
        ...
        this.setPageTitle(this.Translate('add.shipment'))
    ...
    methods: {
        saveShipment () {
            Vue.axios.post('shipments/add/?order=' + this.order.hashid, this.shipment)
                .then((response) => {
                    this.setSnackbarText('Shipment added successfully')
